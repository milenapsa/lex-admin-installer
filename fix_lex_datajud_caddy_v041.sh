...
echo "LEX_DATAJUD_CADDYFIX_V041_START"

docker ps --format '{{.Names}}' | grep -qx 'media-studio-caddy'
docker ps --format '{{.Names}}' | grep -qx 'homosapiens-lex-datajud'

docker exec media-studio-caddy sh -lc "cp /etc/caddy/Caddyfile /tmp/Caddyfile.before-datajud-v041-$(date -u +%Y%m%dT%H%M)"
...
echo "SELFTEST_DATAJUD_CADDYFIX_V041"
...
echo "LEX_DATAJUD_CADDYFIX_V041_OK"
